...
func (o *Client) sendResponses(ctx context.Context, msgs []*chat.ChatCompletionMessage, opts *domain.ChatOptions) (ret string, err error) {
	...
	if opts.ImageFile != "" && !supportsImageGeneration(opts.Model) {
		return "", fmt.Errorf("model '%s' does not support image generation. Supported models: %s\n", opts.Model, strings.Join(ImageGenerationSupportedModels, ", "))
	}
	req := o.buildResponseParams(msgs, opts)
	...
	if err == nil {
		...
		log.Printf("Failed to initialize MCP client: %v\n", err)
	}
	...
	for _, item := range resp.Output {
		if item.Type == "function_call" {
			...
			log.Printf("Added tool results to OpenAI query: %s\n", strResult)
			...
			fmt.Printf("Done. Returning to chat.\n")
		}
	}
